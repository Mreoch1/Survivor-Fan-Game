...
- 2026-05-17: Episode 13 finale results (migration 045): boots Tiffany (F5), Rizo (fire-making), Joe (3rd at FTC); `immunity_winning_player_id = aubry-bracco`; runner-up Jonathan handled in `process-episode` via `SEASON_50_FINALE`. Sole Survivor podium + top 3 fan scores: global `SeasonWinnerBanner` replaces countdown when all 13 episodes have a primary boot (`SeasonTopBanner` in root layout). `npm run process-episode -- 13` after `db:push`.
