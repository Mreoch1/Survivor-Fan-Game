...
- 2026-05-11: Global Season 50 countdown banner on all routes (`SeasonCountdownBanner` in root `layout.tsx`): sticky dramatic strip, episodes remaining = `NEXT_PUBLIC_SEASON_50_TOTAL_EPISODES` (default 14) minus count of Season 50 episodes with `voted_out_player_id` set. Migration 043 adds `anon` SELECT on `episodes` for `season = 50` so unauthenticated pages (login, landing) can load the count.
