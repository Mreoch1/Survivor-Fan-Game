...
- 2026-05-11: Global Season 50 countdown banner on all routes (`SeasonCountdownBanner` in root `layout.tsx`): sticky dramatic strip, episodes remaining = `NEXT_PUBLIC_SEASON_50_TOTAL_EPISODES` (default **13**, CBS-style season length for S50) minus count of Season 50 episodes with `voted_out_player_id` set. Banner also shows “X of Y … saved here” so a stuck zero completed count is obvious. Root `layout.tsx` sets `dynamic = "force-dynamic"` so the count is not served from a stale static shell. Migration 043 adds `anon` SELECT on `episodes` for `season = 50` so unauthenticated pages (login, landing) can load the count; if prod still shows full-season remaining, confirm 043 is applied and env is not forcing an old total.
