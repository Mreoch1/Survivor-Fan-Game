...
const mode=process.argv[2]||"context",options={headers};if(mode==="submit"||mode==="submit-file"){const argument=process.argv[3];if(!argument)throw new Error("Provide structured episode JSON or a JSON file path");const input=mode==="submit-file"?fs.readFileSync(argument,"utf8"):argument;JSON.parse(input);options.method="POST";options.body=input}
const response=await fetch(`${base}/api/automation/commissioner`,options),text=await response.text();if(!response.ok)throw new Error(`Commissioner automation failed (${response.status}): ${text}`);console.log(text);
