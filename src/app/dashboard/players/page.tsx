...
export default async function PlayersPage() {
  ...
  const res =
    resWithMedevac.error && resWithMedevac.error.message?.includes("medevac_player_id")
      ? await supabase
          ...
          .order("episode_number", { ascending: true })
      : resWithMedevac;
  const episodes = (res.data ?? []) as Array<{ episode_number: number; voted_out_player_id?: string | null; second_voted_out_player_id?: string | null; third_voted_out_player_id?: string | null; medevac_player_id?: string | null }>;
  ...
  episodes.forEach((ep) => {
    ...
    if (ep.third_voted_out_player_id) eliminatedByEpisode.set(ep.third_voted_out_player_id, ep.episode_number);
