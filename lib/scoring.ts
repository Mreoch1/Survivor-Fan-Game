...
export function scorePick(input: ScoreInput) {
 const favorite = input.favoriteId && !input.voted.has(input.favoriteId) ? 1 : 0;
 const immunity = !input.immunityVoid && input.immunityWinners.includes(input.immunityPick) ? 2 : 0;
 const boot = input.bootPick && input.voted.has(input.bootPick) ? 3 : 0;
 const bonus = !input.bonusPick || !input.bonusAnswer ? 0 : input.bonusPick === input.bonusAnswer ? 1 : -1;
 const underdog = favorite && !input.departed.has(input.favoriteId) && input.favoriteShare < .2 ? 1 : 0;
 // Shot in the Dark repeats a correct pick's reward, never a Play Your Advantage penalty.
 const doublePoint = input.doubleDown === "immunity" ? immunity : input.doubleDown === "boot" ? boot : input.doubleDown === "bonus" ? Math.max(0, bonus) : 0;
 return { favorite, immunity, boot, bonus, underdog, doublePoint, total: favorite + immunity + boot + bonus + underdog + doublePoint };
}
