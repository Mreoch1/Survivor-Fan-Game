...
export function EpisodeSpotlight({ spotlight }: { spotlight: SeasonDashboard["spotlight"] }) {
  ...
  return <section className="episode-spotlight" aria-label={`Episode ${spotlight.episodeId} league highlights`}>
    ...
    <div className="spotlight-grid">
      <div><span>Episode honors</span><h3>{spotlight.winners.join(" · ") || "A tough week for every camp"}</h3><p>{spotlight.winningPoints > 0 ? `${spotlight.winningPoints} points this round` : "No points awarded this round"}</p></div>
