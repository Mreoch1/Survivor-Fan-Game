...
export function SeasonView({ data }: { data: SeasonDashboard }) {
  ...
  return <>
    ...
    <nav className="season-jump-links" aria-label="Season sections">
      <a href="#score-history">My scorecard</a><a href="#post-merge">Post-merge championship</a><a href="#season-standings">Overall standings</a>
    </nav>
      ...
      <section id="score-history" aria-labelledby="score-history-title">
        ...
        {!latest ? <div className="season-empty"><span aria-hidden="true">◈</span><h3>Your story starts with a pick.</h3><p>Your published episode scores will collect here. Save your opening picks and come back after the first results reveal.</p><a className="button button-primary" href="/play">Make my picks →</a></div> :
          <div className="episode-ledger">{data.history.map((episode, index) => <details className="episode-card" key={episode.episodeId} open={index === 0}>
            <summary><span><small>EPISODE {episode.episodeId}{episode.countsForPostMerge ? " · POST-MERGE" : ""}</small><strong>{episode.title}</strong></span><span className="episode-points">+{episode.points}<small>POINTS</small></span><span className="episode-toggle" aria-hidden="true">⌄</span></summary>
            <div className="episode-detail">
              ...
              {episode.countsForPostMerge && <p className="episode-note">{points(episode.weeklyPoints)} also count toward the post-merge championship.</p>}
            </div>
          </details>)}</div>}
      </section>
      <aside className="season-boards">
        <section id="post-merge" className="season-board post-merge-board" aria-labelledby="post-merge-title"><p className="eyebrow">A fresh race. The same picks.</p><h2 id="post-merge-title">Post-merge championship</h2><p>Starts with the episode after Jeff announces the individual game. Your normal weekly points and bonuses count; Opening Outlast and Final Torch awards stay in the overall competition.</p>
          {data.postMerge.episodesScored ? <><p className="series-status">{data.postMerge.episodesScored} {data.postMerge.episodesScored === 1 ? "episode scored" : "episodes scored"}</p><Standings rows={data.postMerge.standings}/></> : <div className="series-waiting"><strong>{data.postMerge.announcementEpisodeId ? `The announcement was in Episode ${data.postMerge.announcementEpisodeId}.` : "Your second chance is coming."}</strong><span>{data.postMerge.announcementEpisodeId ? "Standings open when the following episode’s results are published." : "Everyone begins this competition at zero. Keep making your regular picks."}</span></div>}
          <p className="board-footnote">Your overall season score keeps growing. Tied scores share the same rank and title.</p>
        </section>
        <section id="season-standings" className="season-board" aria-labelledby="overall-title"><p className="eyebrow">The full season</p><h2 id="overall-title">Overall standings</h2>{data.overallRank ? <Standings rows={data.overall}/> : <p>Standings begin when the first episode’s results are published.</p>}<p className="board-footnote">Weekly points plus Opening Outlast and Final Torch awards.</p></section>
      </aside>
    </div>
