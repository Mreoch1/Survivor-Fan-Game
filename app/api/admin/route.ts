...
export async function GET(){if(!await authorize())return Response.json({error:"Commissioner access required"},{status:403});await ensureDatabase();await publishDueResults();const {data}=await createAdminClient().from("episodes").select("id,title,air_at,lock_at,reveal_at,phase,bonus_question,bonus_options,results_posted,results_published").order("id");return Response.json({episodes:(data||[]).map(e=>({id:e.id,title:e.title,airAt:e.air_at,lockAt:e.lock_at,revealAt:e.reveal_at,phase:e.phase,bonusQuestion:e.bonus_question,bonusOptions:JSON.stringify(e.bonus_options),resultsPosted:e.results_posted?1:0,resultsPublished:e.results_published?1:0})),castaways:castaways.map(c=>({id:c.id,name:c.name,tribe:c.tribe}))})}
...
export async function POST(request:Request){
 ...
 if(body.action==="results"){const result=await recordEpisodeResults({episodeId:Number(body.episodeId),departures:[{castawayId:String(body.booted||""),type:"vote"}],immunityWinners:[String(body.immunityWinner||"")],bonusAnswer:String(body.bonusAnswer||"")});return result.ok?Response.json(result):Response.json({error:result.error},{status:result.status})}
