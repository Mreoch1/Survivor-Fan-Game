...
export async function carryForwardPicks(episodeId:number){
 const db=createAdminClient();const {data:target}=await db.from("episodes").select("id,phase,bonus_options").eq("id",episodeId).eq("results_posted",false).maybeSingle();if(!target)return 0;
 ...
 for(const player of players||[]){if(has.has(player.id))continue;const {data:prior}=await db.from("picks").select("episode_id,favorite_id,immunity_pick,boot_pick,bonus_pick").eq("user_id",player.id).lt("episode_id",episodeId).order("episode_id",{ascending:false}).limit(1).maybeSingle();if(!prior)continue;const favorite=active(prior.favorite_id)?prior.favorite_id:"",immunity=target.phase==="tribe"?(["Savu","Toka"].includes(prior.immunity_pick)?prior.immunity_pick:""):(active(prior.immunity_pick)?prior.immunity_pick:""),boot=active(prior.boot_pick)?prior.boot_pick:"",bonus=options.has(prior.bonus_pick)?prior.bonus_pick:"";if(!favorite&&!immunity&&!boot&&!bonus)continue;const {error}=await db.from("picks").insert({user_id:player.id,episode_id:episodeId,favorite_id:favorite,immunity_pick:immunity,boot_pick:boot,bonus_pick:bonus,double_down:"",carried_from_episode_id:prior.episode_id,updated_at:new Date().toISOString()});if(error)throw error;carried++}
 return carried;
}
