...
import { applyImmunityStreak } from "../lib/scoring";
...
async function recomputePublishedScores(){
 ...
 const [{data:profiles,error:profileError},{data:episodes,error:episodeError},{data:picks,error:pickError},{data:results,error:resultError}]=await Promise.all([
  ...
  db.from("episode_results").select("episode_id,immunity_void,finale_winner,finalists"),
 ]);
 if(profileError||episodeError||pickError||resultError)throw profileError||episodeError||pickError||resultError;
 const published=new Set((episodes||[]).map(e=>e.id)),resultByEpisode=new Map((results||[]).map(r=>[r.episode_id,r]));const finale=(results||[]).filter(r=>r.finale_winner&&published.has(r.episode_id)).sort((a,b)=>b.episode_id-a.episode_id)[0];
 for(const profile of profiles||[]){let total=0,streak=0,longest=0;const updatePromises=[];const userPicks=(picks||[]).filter(p=>p.user_id===profile.id&&published.has(p.episode_id));for(const pick of userPicks){const next=applyImmunityStreak(streak,pick.immunity_point,Boolean(resultByEpisode.get(pick.episode_id)?.immunity_void));streak=next.streak;longest=Math.max(longest,streak);const streakPoint=next.bonus;if(pick.streak_point!==streakPoint)updatePromises.push(db.from("picks").update({streak_point:streakPoint}).eq("id",pick.id));total+=pick.favorite_point+pick.immunity_point+pick.boot_point+pick.bonus_point+pick.underdog_point+streakPoint+pick.double_point}let preseason=0;if(finale){const finalists=Array.isArray(finale.finalists)?finale.finalists.map(String):[];preseason=profile.winner_pick===finale.finale_winner?10:finalists.includes(profile.winner_pick||"")?3:0}await Promise.all(updatePromises);const {error}=await db.from("profiles").update({total_points:total+preseason,preseason_points:preseason,immunity_streak:streak,longest_streak:longest,updated_at:new Date().toISOString()}).eq("id",profile.id);if(error)throw error}
}
