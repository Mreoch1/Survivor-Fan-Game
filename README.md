...
set is_admin = true
where id = (select id from auth.users where email = 'mreoch82@hotmail.com');
```
...
**By user id:** `update public.profiles set is_admin = true where id = 'USER-UUID';`

Migration 008 must be applied first (`npm run db:push`).
